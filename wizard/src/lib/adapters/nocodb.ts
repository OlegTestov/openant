...
const DEFAULT_PROMPTS: Record<string, string> = {
  ArticleTitle: [
    ...
    '4. EMOTIONAL PULL: Include exactly one curiosity or emotional element, chosen to fit THIS specific topic. Pick the most natural category:',
    '   - Concrete number or timeframe ("3 minutes", "in 30 days", "by Monday")',
    '   - Common-mistake framing ("the mistake most people make", "what beginners get wrong")',
    '   - Counterintuitive contrast ("the opposite is true", "why X actually hurts Y")',
    '   - Tangible outcome ("double your X", "without spending Y", "avoid Z")',
    '   - Comparison ("better than X", "the alternative to Y")',
    '   - Authority/proof signal ("backed by research", "what experts do")',
    '   ONE element is enough. Stacking multiple triggers reads as clickbait.',
    ...
    '8. AVOID: Clickbait without substance, ALL CAPS, excessive punctuation, vague promises. AVOID overused curiosity clichés — especially "secret", "hidden", and their direct translations in {language} (e.g. "секретный", "скрытый", "secreto", "oculto", "geheim"). These words are allowed ONLY when literally accurate (the article truly reveals something hidden from the reader). For all other articles, choose triggers from rule #4.',
    '',
  ...
  ArticleText: [
    ...
    '',
    '# INPUT',
    'You will receive a Topic, an optional Description with details, and a Link to embed in the article. The Link may include a resource name in parentheses — if present, you can use that name where appropriate. The Link is the primary conversion goal — it leads to the resource, product, or service the reader should visit.',
    '',
    '# EXPLICIT COUNTS AND LISTS (CRITICAL)',
    '',
    'If the Topic or Description mentions a specific number of items (e.g. "7 steps", "5 rules", "10 prompts", "12 mistakes", "8 ideas", "7 шагов", "5 правил", "10 промптов", "6 ошибок", "8 идей"):',
    '',
    '1. The article MUST contain EXACTLY that number of items — not fewer, not more.',
    '2. Each item MUST be visually distinct. Pick the format that fits the topic:',
    '   - Separate <h2> sections per item ("Step 1: ...", "Rule 1: ...") when items are substantive and deserve dedicated subsections.',
    '   - One <h2> heading followed by a numbered <ol>, where each <li> has a <strong> label and 2–3 sentences of substance, when items are short and parallel.',
    '   In either format every item must be substantive (no bare labels).',
    '3. If the Description enumerates the items themselves (lists them by name or number), use those items verbatim — do not merge, skip, paraphrase away, or replace them with your own.',
    '4. Internal check before writing: re-read the Description, list the items in your head (do NOT include this thinking in the HTML output), then write the article with EXACTLY that count.',
    '5. The article title\'s count and the body\'s count MUST agree. If the title promises "7 steps", the body delivers 7 (not 6, not 8).',
    '',
    'This rule overrides the general "3–6 <h2> subheadings" guideline below whenever explicit counts are present.',
    '',
    'Examples (English for illustration — adapt to {language}):',
    '',
    '  A) Substantive items, one <h2> each:',
    '     Topic: "7 morning routine steps that change your day"',
    '     Body: <h2>Step 1: Wake up without an alarm</h2><p>...</p>',
    '           <h2>Step 2: Drink a full glass of water</h2><p>...</p>',
    '           ... (7 total)',
    '',
    '  B) Parallel short items, one <h2> + <ol>:',
    '     Topic: "5 rules for emails that actually get read"',
    '     Body: <h2>5 rules that make people read your emails</h2>',
    '           <ol>',
    '             <li><strong>Subject line under 50 characters.</strong> 2–3 sentences explaining why.</li>',
    '             <li><strong>One CTA per email.</strong> 2–3 sentences explaining why.</li>',
    '             ... (5 total)',
    '           </ol>',
    '',
    '# ARTICLE STRUCTURE (1000–1500 words, or longer if needed — see LENGTH rule)',
    '',
    ...
    '- Use 3–6 <h2> subheadings. At least 2 of the H2 headings MUST be phrased as natural questions readers would type into a search engine or ask an AI assistant (e.g., "How does X affect Y?", "What are the best Z for W?"). If the EXPLICIT COUNTS rule applies, use as many H2s/items as the count requires — the 3–6 H2 range is for free-form topics only.',
    ...
    '2. LENGTH: 1000–1500 words by default. When the EXPLICIT COUNTS rule applies, each item needs 150–250 words of real substance; expand the total upward proportionally (e.g. ~1700 words for 7 items, ~2200 for 10 items). Cap at ~2500 words. Do not pad — extra length must come from substance, not filler. Include at least 3 bulleted or numbered lists with concrete, actionable items.',
