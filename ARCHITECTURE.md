...
- **Domain mode**: 4 server blocks with auto-HTTPS (main→Ghost, table.X→NocoDB, auto.X→n8n, setup.X→wizard)

---

## n8n workflow templates

Two n8n workflow templates live in `n8n/workflows/`. They are imported into n8n during deploy step 8, with placeholder substitution.
...
| Function | Behavior |
|---|---|
| `generateCaddyfile(domain)` | IP mode (null): single `:80` block → Ghost. Domain mode: 4 server blocks (main→Ghost, table.X→NocoDB, auto.X→n8n, setup.X→wizard) with auto HTTPS. |
...
`getServiceCredentials(setupToken, domain?)` — Generates deterministic admin credentials for Ghost, NocoDB, and n8n. Passwords are derived from `SETUP_TOKEN` via SHA-256 hash (different prefix per service). Admin email defaults to `admin@openant.local` or `admin@{domain}` if a domain is configured. Used by the deploy pipeline and dashboard status endpoint to display login info.
...
| **Deploy** | Deploy button → SSE progress → success/retry | `POST /api/setup/apply` (SSE) | 11-step pipeline with real-time progress. Retry from failed step. Shows service URLs on success. |
...
| `GET /api/setup/status` | Yes | Returns current setup state with masked `api_key` (`***`). |
...
| `POST /api/setup/apply` | Yes | SSE deploy pipeline. Executes 11 steps, streams progress. Supports `?startFrom=N` for retry. |
| `GET /api/dashboard/status` | Yes | Returns health status for all 4 services (Ghost, NocoDB, n8n, Caddy), service URLs, admin credentials, and `saas_mode` flag. |
...
331 tests across 37 test files, all passing:

| File | Tests | What it verifies |
|---|---|---|
| `lib/adapters/__tests__/ghost.test.ts` | 28 | Ghost JWT creation, healthCheck, setup (fast path via JWT, full setup with cookie extraction, EmailError handling, "already configured" recovery), publishPost, getPostUrl, error cases |
| `lib/adapters/__tests__/nocodb.test.ts` | 22 | healthCheck, setup (signup → signin → base → table → columns → sample row, default base deletion), getNextQueued (empty/non-empty, FIFO sort, blank-status filter, field mapping), updateStatus (status + extra fields), getStats (parallel queries, missing pageInfo) |
| `lib/adapters/__tests__/n8n.test.ts` | 27 | healthCheck, setup (fast path with valid key, fallback on invalid key, masked key handling, owner creation, password format), createCredential, importWorkflow (template immutability, all 5 substitution types), activateWorkflow |
...
| `app/api/setup/__tests__/apply.test.ts` | 23 | SSE stream format, all 11 pipeline steps, error handling, startFrom retry, auth, URL generation (domain + IP mode) |
| `lib/__tests__/caddy.test.ts` | 10 | IP-mode and domain-mode Caddyfile generation, writeCaddyfile path handling |
| `lib/__tests__/sse.test.ts` | 4 | createSSEStream, sendSSEEvent format, closeSSE |
| `lib/__tests__/credentials.test.ts` | 7 | Deterministic password generation, admin email from domain, default email, all 3 services returned |
...
The deploy endpoint is an 11-step pipeline that configures all services and streams progress via SSE. Unlike other API routes, it uses `withAuth()` only (not `apiHandler()`) since it returns an SSE stream, not JSON.
...
| 2 | Generate Caddyfile | Creates reverse proxy config (IP or domain mode) |
...
| 6 | Ghost settings | No-op (already configured in step 5) |
| 7 | NocoDB setup | Creates admin account, base, and table; removes default bases; inserts sample row |
| 8 | n8n setup | Auto-provisions API key (fast path: verify existing key) |
| 9 | n8n credentials | Creates 2 credentials (LLM API, NocoDB) |
| 10 | n8n workflows | Imports and activates generate + promote workflows |
| 11 | Finalize | Merges adapter keys into .env, sets `deployed: true` |
...
The `OPENANT_SAAS_MODE` environment variable enables minimal SaaS hooks for future Control Plane integration:
