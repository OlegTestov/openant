...
| 9 | n8n credentials | Creates 2 credentials (LLM API, NocoDB). Managed mode: reads LLM key from env vars instead of wizard state |
| 10 | n8n workflows | Imports and activates generate + promote workflows. Managed mode: reads model from env |
| 11 | Finalize | Merges adapter keys into .env, sets `deployed: true`. Managed mode: excludes n8n from deploy result |
